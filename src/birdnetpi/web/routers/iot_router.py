...
def get_mqtt_service(request: Request) -> MQTTService:
    ...
    if mqtt_service is None:
        raise HTTPException(
            ...
            detail="MQTT service is not available"
        )
    return mqtt_service
...
async def get_mqtt_status(mqtt_service: MQTTService = Depends(get_mqtt_service)):
...
async def test_mqtt_connection(mqtt_service: MQTTService = Depends(get_mqtt_service)):
    """Test MQTT connection and publish a test message."""
    try:
        if not mqtt_service.enable_mqtt:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="MQTT service is disabled"
            )
...
async def add_webhook(
    ...
    try:
        if not webhook_service.enable_webhooks:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Webhook service is disabled"
            )
...
async def get_iot_status(
    ...
    try:
        ...
        return IoTStatusResponse(
            mqtt=MQTTStatusResponse(**mqtt_status),
            webhooks=WebhookStatusResponse(**webhook_status)
        )
