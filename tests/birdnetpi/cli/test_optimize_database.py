...
class TestPrintFunctions:
    ...
    def test_print_query_performance_with_data(self, capsys):
        ...
        performance_data = [
            {"name": "Test Query", "time_ms": 10.5, "rows": 100, "index_ops": 2, "scan_ops": 1},
            {
    ...
    def test_print_query_performance_with_error(self, capsys):
        ...
        performance_data = [{"name": "Failed Query", "error": "Database locked"}]
        print_query_performance(performance_data, "Test Performance")
        captured = capsys.readouterr()
...
class TestCLI:
    ...
    def test_cli_verbose_option(self, mock_optimizer_class, mock_setup_db):
        ...
        with patch("logging.getLogger", autospec=True) as mock_get_logger:
            mock_logger = MagicMock(spec=logging.RootLogger)
