    ...
    def test_load_xlsx_translations(
        ...
        def getitem_side_effect(index):
            if index == 1:  # Header row
                ...
                return [
                    MagicMock(value=row_data[j] if j < len(row_data) else None)
...
class TestDataLoading:
    ...
    def test_load_ioc_data_auto_detect_files(
        self, mock_find_xml, mock_find_xlsx, mock_load_xml, mock_load_xlsx, ioc_service, tmp_path
    ):
    ...
    @patch.object(IOCReferenceService, "_load_xlsx_translations")
    @patch.object(IOCReferenceService, "_load_xml_data")
    def test_load_ioc_data_explicit_files(
        self, mock_load_xml, mock_load_xlsx, ioc_service, tmp_path
    ):
    ...
    def test_complete_workflow(self, mock_load_workbook, mock_parse, tmp_path):
        ...
        def getitem_side_effect(index):
            if index == 1:  # Header row
                ...
                return [
                    MagicMock(value=rows[0][j] if j < len(rows[0]) else None)
                    for j in range(len(headers))
                ]
            return []
