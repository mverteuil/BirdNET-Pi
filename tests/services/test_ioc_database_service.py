...
class TestPopulateFromIOCService:
    ...
    def test_populate_success(self, db_service, mock_ioc_reference_service, capsys):
        ...
        try:
            ...
            robin = session.query(IOCSpecies).filter_by(
                scientific_name="Turdus migratorius"
            ).first()
            ...
            translation = session.query(IOCTranslation).filter_by(
                scientific_name="Turdus migratorius",
                language_code="es"
            ).first()
            ...
            assert metadata_count == 5  # ioc_version, created_at, species_count, translation_count, languages_available
    ...
    def test_populate_with_mocked_datetime(self, mock_datetime, db_service, mock_ioc_reference_service):
...
class TestDatabaseAttachment:
    ...
    def test_attach_to_session(self, populated_db_service):
        ...
        try:
            ...
            result = session.execute(text("SELECT name FROM test_ioc.sqlite_master WHERE type='table'"))
    ...
    def test_attach_with_custom_alias(self, populated_db_service):
        ...
        try:
            ...
            result = session.execute(text(f"SELECT name FROM {alias}.sqlite_master WHERE type='table'"))
            tables = [row[0] for row in result.fetchall()]
...
class TestCreateIOCDatabaseFromFiles:
    ...
    def test_create_ioc_database_from_files(self, mock_service_class, mock_ioc_reference_service, tmp_path):
