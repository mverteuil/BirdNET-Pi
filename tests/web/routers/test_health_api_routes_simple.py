...
class TestHealthEndpoints:
    ...
    def test_health_root_endpoint(self, client):
        """Should return healthy status for root health endpoint."""
    ...
    def test_health_live_endpoint(self, client):
        """Should return alive status for liveness endpoint."""
    ...
    def test_health_ready_endpoint(self, client):
        """Should return readiness status with database check."""
    ...
    def test_health_detailed_endpoint(self, client):
        """Should return detailed health information with components."""
...
class TestHealthResponseFormat:
    ...
    def test_consistent_timestamp_format(self, client):
        """Should have consistent timestamp format across all health endpoints."""
        endpoints = [
            "/api/health/",
            "/api/health/detailed",
        ]
    ...
    def test_version_included(self, client):
        """Should include version in appropriate endpoints."""
...
class TestHealthDatabaseCheck:
    ...
    def test_ready_endpoint_checks_database(self, client):
        """Should check database connectivity in ready endpoint."""
        response = client.get("/api/health/ready")
        data = response.json()
    ...
    def test_detailed_endpoint_includes_database(self, client):
        """Should include database check in detailed endpoint."""
        response = client.get("/api/health/detailed")
...
class TestHealthErrorHandling:
    ...
    def test_health_endpoints_handle_errors_gracefully(self, client):
        """Should never crash the application in health endpoints."""
        endpoints = [
            "/api/health/",
            "/api/health/live",
            "/api/health/ready",
    ...
    def test_invalid_health_endpoint_returns_404(self, client):
        """Should return 404 for invalid health endpoints."""
        invalid_endpoints = [
            "/api/health/invalid",
