...
daily = st.sidebar.checkbox(
    "Single Day View",
    help="Select if you want single day view, unselect for multi-day views",
)
...
if start_date == end_date:
    ...
    resample_sel = st.sidebar.radio(
        "Resample Resolution",
        ("Raw", "15 minutes", "Hourly", "DAILY"),
        index=1,
        help="Select resolution for species - DAILY provides time series",
    )
...
if daily is False:
    ...
    if resample_time != "1D":
        ...
        if specie == "All":
            ...
            with col1:
                ...
                st.subheader(
                    ...
                    + str(
                        "{:.2f}%".format(
                            np.median(df2[df2["Com_Name"] == specie]["Confidence"])
                            * 100
                        )
                    )
                )
            ...
            with col2:
                try:
                    recording = st.selectbox(
                        "Available recordings", recordings.sort_index(ascending=False)
                    )
                    date_specie = df2.loc[
                        df2["File_Name"] == recording, ["Date", "Com_Name"]
                    ]
            ...
            if seen:
                with colc:
                    verified = st.radio(
                        "Verification", ["True Positive", "False Positive"]
                    )
                    ...
                    if verified == "False Positive":
                        ...
                        df_unknown = pd.DataFrame(
                            {"Sci_Name": ["UNKNOWN"], "Com_Name": ["UNKNOWN"]}
                        )
                        df_names = pd.concat([df_unknown, df_names], ignore_index=True)
                        with cold:
                            corrected = st.selectbox(
                                "What species?", df_names["Com_Name"]
                            )
        ...
        selected_pal = st.sidebar.selectbox(
            "Select Color Pallet for Daily Detections", color_pals
        )
        ...
        sunrise_week_list, sunrise_list, sunrise_text_list = sunrise_sunset_scatter(
            num_days_to_display
        )
    ...
    fig.add_trace(
        go.Heatmap(
            x=heat.columns,
            y=heat.index,
            z=heat_values_normalized,  # heat.values,
            showscale=False,
            text=labels,
            texttemplate="%{text}",
            colorscale="Blugrn",
