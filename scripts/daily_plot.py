...
f, axs = plt.subplots(
    1, 2, figsize=(10, 4), gridspec_kw=dict(width_ratios=[3, 6]), facecolor="#77C487"
)
...
plot = sns.countplot(
    y="Com_Name", data=df_plt_top10_today, palette=colors, order=freq_order, ax=axs[0]
)
...
# Generate frequency plot
plot = sns.countplot(
    y="Com_Name", data=df_plt_Bot10_today, palette=colors, order=freq_order, ax=axs[0]
)
...
savename = (
    userDir
    + "/BirdSongs/Extracted/Charts/Combo2-"
    + str(now.strftime("%Y-%m-%d"))
    + ".png"
)
plt.savefig(savename)
