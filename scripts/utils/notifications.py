...
def sendAppriseNotifications(
    ...
    if os.path.exists(APPRISE_CONFIG) and os.path.getsize(APPRISE_CONFIG) > 0:
        ...
        APPRISE_ONLY_NOTIFY_SPECIES_NAMES = settings_dict.get(
            "APPRISE_ONLY_NOTIFY_SPECIES_NAMES"
        )
        if (
            APPRISE_ONLY_NOTIFY_SPECIES_NAMES is not None
            and APPRISE_ONLY_NOTIFY_SPECIES_NAMES.strip() != ""
        ):
            if any(
                bird.lower().replace(" ", "") in comName.lower().replace(" ", "")
                for bird in APPRISE_ONLY_NOTIFY_SPECIES_NAMES.split(",")
            ):
                return

        APPRISE_ONLY_NOTIFY_SPECIES_NAMES_2 = settings_dict.get(
            "APPRISE_ONLY_NOTIFY_SPECIES_NAMES_2"
        )
        if (
            APPRISE_ONLY_NOTIFY_SPECIES_NAMES_2 is not None
            and APPRISE_ONLY_NOTIFY_SPECIES_NAMES_2.strip() != ""
        ):
            if not any(
                bird.lower().replace(" ", "") in comName.lower().replace(" ", "")
        ...
        if APPRISE_MINIMUM_SECONDS_BETWEEN_NOTIFICATIONS_PER_SPECIES != "0":
            if species_last_notified.get(comName) is not None:
                try:
                    if int(timeim.time()) - species_last_notified[comName] < int(
                        APPRISE_MINIMUM_SECONDS_BETWEEN_NOTIFICATIONS_PER_SPECIES
                    ):
                        return
