...
with open(userDir + "/BirdNET-Pi/scripts/thisrun.txt", "r") as f:
    ...
    try:
        model = str(
            str(
                str([i for i in this_run if i.startswith("MODEL")]).split("=")[1]
            ).split("\\")[0]
        )
        sf_thresh = str(
            str(
                str([i for i in this_run if i.startswith("SF_THRESH")]).split("=")[1]
            ).split("\\")[0]
        )
    except Exception:
        ...
        for d in detections:
            for entry in detections[d]:
                if entry[1] >= min_conf and (
                    (entry[0] in INCLUDE_LIST or len(INCLUDE_LIST) == 0)
                    and (entry[0] not in EXCLUDE_LIST or len(EXCLUDE_LIST) == 0)
                    and (
                        entry[0] in PREDICTED_SPECIES_LIST
            ...
            for d in detections:
                ...
                for entry in detections[d]:
                    if entry[1] >= min_conf and (
                        ...
                        if entry[0] not in species_apprised_this_run:
                            settings_dict = config_to_settings(
                                userDir + "/BirdNET-Pi/scripts/thisrun.txt"
                            )
                        ...
                        if birdweather_id != "99999":
                            try:
                                ...
                                post_soundscape_id = (
                                    '"soundscapeId": ' + str(soundscape_id) + ","
                                )
